...
contract NFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function mintERC20(string memory tokenURI) public payable {
        bool success = hpToken.transferFrom(msg.sender, address(this), costERC20);
        if (!success) {
            revert NFT__TransferFailed();
        }
        balances[msg.sender] += costERC20;
        ...
        _mint(msg.sender, newItemId);
        _setTokenURI(newItemId, tokenURI);
        emit Erc20Mint(msg.sender, costERC20);
    }
